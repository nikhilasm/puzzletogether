 ...
 *
 * This has now cost the build eight separate times across four phases, and once
 * ([TODO.md](../docs/TODO.md), Phase 4b) it did *not* cost the build — the remainder of the file
 * happened to parse as valid JavaScript, `npm run build` succeeded, and every browser test failed at
 * once on an error that pointed nowhere near the CSS. That is the case this exists for. A truncated
 * stylesheet is not a syntax error often enough to rely on being one.
 *
 * The scan is the trap stated literally: find each `css` template, walk forward to the backtick that
 * closes it, and fail if that backtick is inside a comment — because then it is not closing the
 * template, it is ending it early and the rest of the stylesheet is being read as code.
 *
 ...
 *
 * Interpolations are skipped whole — `${focusRing}` is JavaScript, and a backtick inside one would
 * be a nested template rather than a terminator. Everything else is scanned in one pass, tracking
