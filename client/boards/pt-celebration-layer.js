 ...
 * stays smooth, and a celebration is not worth a branch in the grid's hot path.
 *
 ...
 * (brand.md §3): this is a wash over the grid's *surface*, the same channel the cursor already
 * uses, and for the length of the wave it is the *only* thing using it, since the cursor's own
 * washes stand down while it runs (see #renderCell on <pt-board>).
 */
 ...
 * over.
 *
 * The sum of the two motion tokens the stylesheet below reads, the sweep and then the last
 * square's own pulse, restated here because a setTimeout cannot read a duration out of a stylesheet.
