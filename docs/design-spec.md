...
| suguru | `{ regions: [{ id, cells: [idx] }], alphabet: '123456' }` | digit or null, plus `marks[]` |
...
**Suguru is the first type where `meta.alphabet` is not what every cell accepts.** A region's size caps its own digits, 1..size, so a 3-cell region and a 6-cell region on the same grid answer to different bounds. `meta.alphabet` keeps meaning the puzzle's full display range, since that is still what sizes the keypad and the mark grid; the per-cell bound is derived from `meta.regions` and cached rather than stored, on both sides ([ADR-0019](adr/0019-a-region-bounds-its-own-alphabet.md)). Suguru also drops the row/column rule every other digit type has: only its region and the no-touch adjacency rule constrain a cell, so `isHighlighted` washes the focused cell's own region rather than a row and column, the same hook kakuro already overrides for its two crossing runs, no new `<pt-board>` hook.
...
| `isHighlighted` (suguru) | the focused cell's own region, since suguru has no row/column rule | suguru |
...
**Suguru.** Partition and fill happen together, unlike every other generated type here: a region's size caps which digits are legal inside it, so borrowing digits from a pre-solved grid the way kenken does can hand a region a digit it is not allowed to hold. Lay out a region partition with the same flood-fill approach kenken's cage builder uses, sized to a per-difficulty distribution, then fill that fixed partition with a backtracking search whose domain per cell is 1..(its own region's size), pruned by the no-touch adjacency rule at every placement. That full grid is then dug like sudoku's: holes removed one cell at a time, keeping a removal only when a counting solver still proves the puzzle unique, since a suguru with no givens at all is essentially never unique on its own. A partition the fill search cannot complete, or a dig that cannot reach the requested difficulty, is discarded and redrawn with a fresh seed rather than repaired in place ([ADR-0020](adr/0020-suguru-is-drawn-and-verified-not-tightened.md)); the pool's existing redraw budget absorbs the cost ([ADR-0017](adr/0017-the-pool-redraws-for-difficulty.md)).
