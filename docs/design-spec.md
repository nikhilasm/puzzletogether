...
- **The room panel** — room code, seat count, and roster grouped inside one rule. **Revised after Phase 4**: the three were a stack of centred lines with nothing saying they belonged together, and the roster in particular read as a screen above the real one. The code sits at the panel's top left and the count `x/n` at its top right, centred against it rather than sharing its baseline — the two are several steps apart on the type scale, and neither is prose. The count is drawn by `<pt-app>` rather than by `<pt-player-chips>`: it is a fact about the room, and it leaves the roster free to be nothing but the roster. It is unlabelled, because a bare `2/8` at the head of the roster it counts cannot mean anything else and the word "Players" was saying it twice. **A rule, no fill** — a panel on `--paper-raised` read as a card to be dealt with rather than as a caption on the room — and it sits close under the wordmark and close over the grid, which is the thing the page is actually about.
- **`Room kqjy`** — DM Mono, `--accent`. **Room codes are 4 lowercase characters.**
- **Players** — chips two per row, each name in that player's color, the host's chip carrying a leading ★ and your own a trailing quiet `you`. Colour is carried by the name and nothing else: a colour bar down each chip's leading edge was built and taken out again, because ten of them stacked in a two-column grid read as a rack of tabs, and the roster is a list of people rather than a legend for the grid. Disconnected players dim during the grace period before dropping off.
  - **Your own chip is a button**: it opens a palette of the ten colors, with the ones other players hold greyed and struck through — shown rather than hidden, so you can see what the room already holds. Color is the only piece of identity a player can change, so it is edited where it is shown rather than behind a settings screen, and the server, not the disabled attribute, is what enforces uniqueness.
...
- **The grid** — heavy cage/region borders, small cage or clue labels in each cell's top-left.
  ...
  - **Presence is a segmented stripe along the cell's bottom edge**, one equal segment per player focused there, sized from `--cell-size` and clamped to 3–6px. **Revised after Phase 4** from colored dots in the cell's top-right, capped at 3 with a `+n`. Both of that design's faults were one fault: presence was drawn *in* the cell's content area at a size that grew with the room, so it covered the top-right pencil mark and ran out of room at four players. An edge cannot collide with a note or a value, and a fixed footprint that subdivides has no fourth-player problem — eight players are eight thin bands, and reading "how crowded is this square" off a count of colours is faster than counting dots, which is what the `+n` was quietly admitting. The overlay declares **both grid axes**: with only its columns named, every row past the first was an implicit track sized to its contents, and a stripe below the top row landed nowhere near its cell.
  - **Your own cursor is drawn in your own color.** The board publishes the local player's `--player-N` as `--focus-color`, and the selection wash and the lighter wash on the cells it implies — a sudoku's row and column, a crossword's entry — are both mixed from it. Two people over one screen can then tell whose cursor is whose, and a player's colour means the same thing in the roster, on the grid, and in everyone else's presence stripe. It falls back to `--accent`, which is what every cursor was before. **The line it does not cross** is in [brand.md §3](brand.md): a wash says who is *looking*, which is presence; a coloured digit would say who *wrote* it, and attribution never touches puzzle content.
...
- **`Notes` toggle** — input mode: pencil marks vs. entering values. **Revised in Phase 2** from the mock's `Notes | Solve` segmented pair to a single control above the keypad: two segments implied two independent things to choose between when there is really one setting, of which Solve is simply the off state — and the control belongs with the digits whose meaning it changes, not down among Check and Reveal. **Revised again after Phase 4** from a `role="switch"` to an `aria-pressed` button, along with Rebus and the brushes, so that every setting in the app is the one control the nonogram brushes had always been ([ADR-0011](adr/0011-icons-in-the-panel-words-in-the-page.md)). It keeps its word: the same revision dropped the panel's labels and the next one put them back, under the icons rather than beside them ([ADR-0012](adr/0012-a-label-under-every-icon.md)).
...
- **The puzzle-action row** — `Puzzle Select · Check · Reveal · Leave room`, under a hairline, each with an icon **and its word**. **Scope is what divides the controls from the panel's, not frequency.** Erase and Undo stay in the panel because they act on the cell you are in; these act on the room's whole puzzle or on your seat in it, and two are host-only, so nobody reaches for Undo and lands on Reveal. The row renders only the actions a given player has. It no longer disappears, because Leave room is in it and every player has that.
  - **Frequency is what decides the words**, which is the other half of the same rule: these are pressed once or twice a puzzle, so an unlabelled icon here would be a guess, where in the panel it is learned in the first minute ([ADR-0011](adr/0011-icons-in-the-panel-words-in-the-page.md)).
  - With a host's full set the four come to about 618px against the row's 480px, so **Leave room wraps to its own line** beneath the other three. Same size, same rule, same reading order — a second line, not a second kind of control.
- **`Leave room`** — on this screen and on Puzzle Select both, last, and marked in `--danger`. **Revised after Phase 4**: it used to sit below the row at a smaller size, which was meant to read as quiet and read as an afterthought — it was the only button on the screen at its own size. It is now the same button as the ones beside it and says what it is with colour, which is the channel that does not cost it a tap target. Leaving is still not something you should have to abandon a puzzle to do.
- **Footer** — hairline rule, then the app's own controls: a **theme button** and an **About** button as icon buttons on one line, with **GitHub** and **Report an issue** as small links side by side beneath. The theme control is an *action*, not a toggle — it names the switch it would make and wears the icon of the theme it would leave you in. **Revised after Phase 4** from a labelled switch, a version line, and a link stacked down the page. The division is that a button changes the app and a link leaves it; the version line moved into About, which is where the rest of the answer to that question now lives.

Actions are buttons and settings are pressed buttons — see [brand.md §4](brand.md) for why the distinction is kept literal in the markup, and for the icon, danger, and focus-ring rules that go with it.

**The input panel's button bar is one row, always, and every control in it carries a one-word label under its icon.** The controls *share* the row (`flex: 1 1 0`) rather than each taking the width of its own word, which is what lets four of them fit a 320px screen with the words still on. Their icons are a fixed `1rem` rather than `iconStyle`'s `1.25em`, so they match each other rather than whichever shadow root's font size they happen to inherit. → [ADR-0012](adr/0012-a-label-under-every-icon.md)
...
**A banked type is browsed rather than described ([ADR-0009](adr/0009-a-bank-is-browsed-not-described.md)) — and a browsed list can be filtered.** The two are not in tension: the filters narrow a list of puzzles that exist, where the three questions asked a host to describe one that might not. Size and difficulty are the axes, because they are the only two facts every banked puzzle carries, and the card states both so that what is being filtered on is visible on the thing being filtered.

Three rules keep the filters from becoming the interface again:

- **A row appears only when it has more than one value behind it**, which is the rule the puzzle-type row already follows. On the tracked bank — four minis, all 5×5, all easy — neither row is drawn and the list is the only thing on screen. This is the answer to the objection that deferred filters in ADR-0009: they were rejected as clutter at four puzzles, and at four puzzles they are still not drawn.
- **An option that would empty the list is disabled, not hidden**, greyed like a colour another player holds. That is also what *guarantees* the list is never empty: an option is pressable only if something is behind it given the other filter's current value, so every pair a host can reach holds at least one card, and Start can never be left pointing at a puzzle that is no longer on screen. Filtering the chosen card away moves the selection to the first one still visible.
- **The count appears only while a filter is on** — `Choose a puzzle · 2 of 12`. A list that silently got shorter is the failure; how much of the bank is hidden is what tells a host whether to widen it back out.

**The list gets the whole column; the option rows keep a reading measure.** A card carries a title, an author, and a publication, which is more text than a row of buttons and wraps badly at the width a row of buttons wants. So `<pt-puzzle-picker>` is given the full 40rem in Puzzle Select and in the congrats modal both, and caps its own `fieldset`s at 28rem and centres them — 28 rather than 26 because that is where the four puzzle types stop wrapping, and `Crossword` alone on a second line read as a different kind of choice from the other three.
...
3. **The keys** — digits `1..n` sized to the puzzle's alphabet, or the three **QWERTY** rows, letters only. Every key is one width and the short rows are centred against the ten-key row above; ⌫ used to take the corner of the bottom row and has moved up to the button bar ([ADR-0012](adr/0012-a-label-under-every-icon.md)).
...
**Nonogram takes the same slot with `Fill · Cross · Erase`** — a tri-toggle, because picking a brush changes what the grid does next rather than changing the grid, the same distinction that makes Notes a setting and Check an action. It replaces *both* the Notes toggle and the digits: a nonogram has no digits to press, and no pencil marks either, because the cross **is** the note. It borrowed the `aria-pressed` option-group pattern from the puzzle pickers because three mutually exclusive states is one more than a switch could hold — and **after Phase 4 that pattern is the app's only one**, with Notes, Rebus, and the theme control all arriving at it ([ADR-0011](adr/0011-icons-in-the-panel-words-in-the-page.md)).

**Erase leaves the row for nonogram and for crossword; Undo never does.** Erase clears the selected cell — the counterpart to pressing a digit into it — so a puzzle with no digits has no use for it, and nonogram erases by dragging with its erase brush. Crossword's counterpart is **Backspace, which sits in the button bar with it**. Undo belongs to every type there will ever be.

**Backspace is a button-bar control, not a key.** It was in the pad's bottom-right corner because that is where a phone keyboard puts it — but a phone keyboard's ⌫ is a key among keys, and ours clears a square and steps back along the entry, which is what Erase and Undo do and not what pressing a letter does. The pad is letters and nothing else.

**Crossword's bar reads Clues · Rebus · Undo · Backspace.** Clues leads because it is the only control there that does not act on the square you are on: it opens the puzzle's other half. Backspace ends it, nearest the letters it corrects — it is the one control in the bar a solver reaches for mid-word, where Undo is occasional.
...
**A square holds one letter unless the player says otherwise.** Typing replaces the square's contents and moves on. With **Rebus** on, typing appends instead, and the square holds a whole word — the device a themed crossword is often built around ([ADR-0007](adr/0007-rebus-widens-the-cell-value.md)). It is a toggle in the panel's button bar, which is where every type keeps its one setting — Notes for the digit puzzles, the brush bar for nonogram. Holding <kbd>Shift</kbd> does the same thing for one keystroke on a physical keyboard; **on a touch screen the toggle is the only path**, since our pad has no Shift key to offer and a phone's own produces an uppercase letter and no separate signal. That is why it has to hold its state visibly, which the `aria-pressed` border and wash do.
...
- <kbd>Backspace</kbd> **takes the letter out and steps back**, so a held key walks a wrong answer out of the grid a square at a time. It never leaves the entry: at the first square it empties it and stops. On a rebus square mid-assembly it peels one character and **keeps** the cursor, because that is the correction the player means. <kbd>Delete</kbd> is the same rule minus the step. The panel's own Backspace button is the same act, asked of the board rather than reimplemented — and it is why crossword has no Erase control, the same reasoning that took Erase off nonogram's brush bar.
...
**Black squares are `--ink` and carry no number, no cursor, and no presence stripe.** They already render this way; what Phase 4 adds is that they stop being reachable. **Circled squares** — `GEXT` in the source files, and where a themed puzzle usually hides its bonus answer — draw as a thin ring inset in the square, in `--graphite` so they read as an annotation on the grid rather than as a value in it.

**The clue list is synchronized with your own cursor, not with the room's.** It follows your position and direction and scrolls the current clue into view; where everyone else is working stays where it already is, on the grid as presence stripes. Adding direction to `game:focus` would let the list say precisely which entry each player is on, and it is deliberately not built: it is a protocol change in the phase whose premise is that a new type needs none, bought for a nicety. Recorded as a candidate for later, once real play says whether the stripes are enough.
...
└─ client/
   ...
   │  # pt-switch is gone: every setting is an aria-pressed icon button now (ADR-0011)
...
- **Theme**: `data-theme` on `<html>` plus the brand custom properties; the footer's theme toggle persists to `localStorage`, initial value respects `prefers-color-scheme`, and an inline bootstrap in `index.html` applies it before first paint so a dark-preferring visitor never sees a flash of cream.
...
- **The catalog grows with the bank, and nothing bounds it.** Every player receives every puzzle's title, author, and source on join, including players who cannot start anything. Four minis is nothing and a thousand crosswords is a payload; there is no pagination and no search, and at some size there will have to be. The size and difficulty filters do not help here — they narrow what is *drawn*, out of a catalog that was already sent whole. → [ADR-0009](adr/0009-a-bank-is-browsed-not-described.md)
...
- **Co-op sync**: `npm run dev`, two browsers, create + join, type into the same cell from both — last writer wins in both views, presence stripes track, no desync. Kill one client's network briefly and confirm gap recovery via `sync:request` snapshot.
